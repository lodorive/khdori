	...
	//연결 후 해야할 일들을 콜백함수로 지정(onopen, onclose, onerror, onmessage)
		...
		if (data.clients) { // 목록 처리
		    ...
		    for (var i = 0; i < data.clients.length; i++) {
		        var memberId = data.clients[i].memberId;
		        var memberLevel = data.clients[i].memberLevel;
		        var memberNickname = data.clients[i].memberNickname;
			...
			var memberNickname = data.memberNickname;
			   ...
			   if (isDM) {
				...
				if (data.memberId === memberId) {			
				    ...
				    if (isDM) {
				        messageContainer.css("background-color", "#FCFCFD"); // 디엠 메시지 배경색 변경
				    }
