...
//		clients.remove(session);
		...
		//모든 접속자에게 접속자 명단을 전송
		sendClientList(); 
		sendMessageList(client); //접속자에게 메세지 이력 전송 
	}
...
//		data.put("clients", clients); //전체회원명단(null이 문제가 됨)
	...
	public void sendMessageList(ClientVO client) throws IOException {
		...
		for(ChatDto dto : list) {
			...
			else {//접속한 사람이 받은 메세지라면 4개 데이터 전송
				Map<String, Object> map = new HashMap<>();
				map.put("content", dto.getChatContent());
				map.put("memberId", dto.getChatSender());
				map.put("memberLevel", dto.getChatSenderLevel());
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		//사용자가 보낸 메세지를 key,value를 알아서 잘라서 map에 넣어라
...
//		log.debug("params ={}", params);
		...
		if(isDM) { //DM일 경우
			Map<String, Object> map = new HashMap<>();
			map.put("dm", true);
			map.put("memberId", client.getMemberId()); //보낸사람
