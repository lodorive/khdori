...
//		session.setAttribute("tid", response.getTid());
	...
	public String test3Purchase(@ModelAttribute PurchaseListVO listVO, Model model) {
		...
		for(PurchaseVO vo : purchaseList) { //사용자가 선택한 번호와 수량의 목록을 반복하며
			ProductDto productDto = productDao.selectOne(vo.getProductNo()); //상품 정보를 구한다
...
//		int paymentNo = paymentDao.sequence();
		...
		paymentDao.insert(PaymentDto.builder()
					...
					.paymentRemain(response.getAmount().getTotal()) //총 취소 가능액
				.build());		
		//[3] 상품 개수만큼 결제 상세 정보를 등록
		...
		for(PurchaseVO vo : list) {
			...
			paymentDao.insertDetail(PaymentDetailDto.builder()
						.paymentDetailOrigin(paymentNo) //상위 결제 번호
						.paymentDetailProduct(vo.getProductNo()) //상품 번호
						.paymentDetailProductName(productDto.getProductName()) //상품명
						.paymentDetailProductPrice(productDto.getProductPrice()) //상품 가격
						.paymentDetailProductQty(vo.getQty()) //수량
					.build());
		}
...
//		model.addAttribute("list", paymentDao.selectTotalList()); //전체내역
	...
	public String test3cancelAll(@RequestParam int paymentNo) throws URISyntaxException {
		//[1] 전달 받은 paymentNo 로 paymentDto를 조회
		PaymentDto paymentDto = paymentDao.selectOne(paymentNo);
		if(paymentDto == null || paymentDto.getPaymentRemain() == 0) {// 잔여금액이 0이라면 
			throw new NoTargetException("이미 취소된 결제입니다"); //차단
		}
	...
	//결제 취소와 결제 실패의경우에도 세션에 저장한 flash value를 삭제해야 한다
